...
//	                Export.Export2Excel(nodes, openfileDlg.FileName);
        ...
        public static void Export2CSV(MyTreeNode node, string outputfile)
        {
            ...
            MainWindow.notify.SetStatusMessage("");
            MainWindow.notify.SetProcessBarVisible(false);
        }
...
//            allLines.Append("\r\n");
            ...
            if (row % step == 0)
            {
                MainWindow.notify.SetProcessBarValue((int)(100.0 * row / allRow));
                MainWindow.notify.SetStatusMessage("正在导出第" + row + "个节点（总共" + allRow + "个节点）");
            }
