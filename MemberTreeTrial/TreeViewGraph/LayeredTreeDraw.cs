...
namespace TreeContainer
{
	...
	public class LayeredTreeDraw
	{
        ...
        {
            get { return Info(_tnRoot).SubTreeWidth; }
        }
		...
		static public IEnumerable<T> VisibleChildrens<T>(ITreeNode tn)
		{
			foreach (ITreeNode tnCur in tn.TreeChildren)
			{
				if (!tnCur.Collapsed)
				{
					foreach (T item in VisibleChildrens<T>(tnCur))
					{
						yield return item;
					}
				}
		...
		static public IEnumerable<T> Childrens<T>(ITreeNode tn)
		{
			foreach (ITreeNode tnCur in tn.TreeChildren)
			{
				foreach (T item in Childrens<T>(tnCur))
				{
					yield return item;
				}
				yield return (T)tnCur;
